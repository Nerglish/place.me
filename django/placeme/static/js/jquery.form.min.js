...
e.timeout&&(x=setTimeout(function(){s=!0;g(v)},e.timeout));var j=[];try{if(e.extraData)for(var m in e.extraData)e.extraData.hasOwnProperty(m)&&j.push(b('<input type="hidden" name="'+m+'">').attr("value",e.extraData[m]).appendTo(i)[0]);e.iframeTarget||(t.appendTo("body"),o.attachEvent?o.attachEvent("onload",g):o.addEventListener("load",g,!1));setTimeout(a,15);i.submit()}finally{i.setAttribute("action",h),c?i.setAttribute("target",c):k.removeAttr("target"),b(j).remove()}}function g(a){if(!h.aborted&&
!z){try{q=o.contentWindow?o.contentWindow.document:o.contentDocument?o.contentDocument:o.document}catch(c){p("cannot access response document: ",c),a=u}if(a===v&&h)h.abort("timeout");else if(a==u&&h)h.abort("server abort");else if(q&&q.location.href!=e.iframeSrc||s){o.detachEvent?o.detachEvent("onload",g):o.removeEventListener("load",g,!1);var a="success",d;try{if(s)throw"timeout";var f="xml"==e.dataType||q.XMLDocument||b.isXMLDoc(q);p("isXml="+f);if(!f&&(window.opera&&(null===q.body||!q.body.innerHTML))&&
...
r)e.extraData=e.extraData||{},e.extraData[c]=r;e.forceSync?f():setTimeout(f,10);var w,q,B=50,z,C=b.parseXML||function(a,b){if(window.ActiveXObject){b=new ActiveXObject("Microsoft.XMLDOM");b.async="false";b.loadXML(a)}else b=(new DOMParser).parseFromString(a,"text/xml");return b&&b.documentElement&&b.documentElement.nodeName!="parsererror"?b:null},E=b.parseJSON||function(a){return window.eval("("+a+")")},D=function(a,c,d){var e=a.getResponseHeader("content-type")||"",f=c==="xml"||!c&&e.indexOf("xml")>=
...
typeof c?b.trim(c):"")||window.location.href||"")&&(c=(c.match(/^([^#]+)/)||[])[1]);a=b.extend(!0,{url:c,success:b.ajaxSettings.success,type:d||"GET",iframeSrc:/^https/i.test(window.location.href||"")?"javascript:false":"about:blank"},a);c={};this.trigger("form-pre-serialize",[this,a,c]);if(c.veto)return p("ajaxSubmit: submit vetoed via form-pre-serialize trigger"),this;if(a.beforeSerialize&&!1===a.beforeSerialize(this,a))return p("ajaxSubmit: submit aborted via beforeSerialize callback"),this;var m=
a.traditional;void 0===m&&(m=b.ajaxSettings.traditional);var l=[],i,j=this.formToArray(a.semantic,l);a.data&&(a.extraData=a.data,i=b.param(a.data,m));if(a.beforeSubmit&&!1===a.beforeSubmit(j,this,a))return p("ajaxSubmit: submit aborted via beforeSubmit callback"),this;this.trigger("form-submit-validate",[j,this,a,c]);if(c.veto)return p("ajaxSubmit: submit vetoed via form-submit-validate trigger"),this;c=b.param(j,m);i&&(c=c?c+"&"+i:i);"GET"==a.type.toUpperCase()?(a.url+=(0<=a.url.indexOf("?")?"&":
...
b.fn.formToArray=function(a,f){var g=[];if(0===this.length)return g;var d=this[0],c=a?d.getElementsByTagName("*"):d.elements;if(!c)return g;var k,m,l,i,j,n;k=0;for(n=c.length;k<n;k++)if(j=c[k],l=j.name)if(a&&d.clk&&"image"==j.type)!j.disabled&&d.clk==j&&(g.push({name:l,value:b(j).val(),type:j.type}),g.push({name:l+".x",value:d.clk_x},{name:l+".y",value:d.clk_y}));else if((i=b.fieldValue(j,!0))&&i.constructor==Array){f&&f.push(j);m=0;for(j=i.length;m<j;m++)g.push({name:l,value:i[m]})}else if(u&&"file"==
j.type&&!j.disabled)if(f&&f.push(j),i=j.files,i.length)for(m=0;m<i.length;m++)g.push({name:l,value:i[m],type:j.type});else g.push({name:l,value:"",type:j.type});else null!==i&&"undefined"!=typeof i&&(f&&f.push(j),g.push({name:l,value:i,type:j.type,required:j.required}));if(!a&&d.clk&&(c=b(d.clk),k=c[0],(l=k.name)&&!k.disabled&&"image"==k.type))g.push({name:l,value:c.val()}),g.push({name:l+".x",value:d.clk_x},{name:l+".y",value:d.clk_y});return g};b.fn.formSerialize=function(a){return b.param(this.formToArray(a))};
...
b.fieldValue=function(a,f){var g=a.name,d=a.type,c=a.tagName.toLowerCase();void 0===f&&(f=!0);if(f&&(!g||a.disabled||"reset"==d||"button"==d||("checkbox"==d||"radio"==d)&&!a.checked||("submit"==d||"image"==d)&&a.form&&a.form.clk!=a||"select"==c&&-1==a.selectedIndex))return null;if("select"==c){var k=a.selectedIndex;if(0>k)return null;for(var g=[],c=a.options,m=(d="select-one"==d)?k+1:c.length,k=d?k:0;k<m;k++){var l=c[k];if(l.selected){var i=l.value;i||(i=l.attributes&&l.attributes.value&&!l.attributes.value.specified?
l.text:l.value);if(d)return i;g.push(i)}}return g}return b(a).val()};b.fn.clearForm=function(a){return this.each(function(){b("input,select,textarea",this).clearFields(a)})};b.fn.clearFields=b.fn.clearInputs=function(a){var f=/^(?:color|date|datetime|email|month|number|password|range|search|tel|text|time|url|week)$/i;return this.each(function(){var g=this.type,d=this.tagName.toLowerCase();if(f.test(g)||"textarea"==d)this.value="";else if("checkbox"==g||"radio"==g)this.checked=!1;else if("select"==
d)this.selectedIndex=-1;else if(a&&(!0===a&&/hidden/.test(g)||"string"==typeof a&&b(this).is(a)))this.value=""})};b.fn.resetForm=function(){return this.each(function(){("function"==typeof this.reset||"object"==typeof this.reset&&!this.reset.nodeType)&&this.reset()})};b.fn.enable=function(a){void 0===a&&(a=!0);return this.each(function(){this.disabled=!a})};b.fn.selected=function(a){void 0===a&&(a=!0);return this.each(function(){var f=this.type;"checkbox"==f||"radio"==f?this.checked=a:"option"==this.tagName.toLowerCase()&&
